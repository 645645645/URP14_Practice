...
namespace Unity.Collections
{
    ...
    public unsafe struct SortJobDeferByRefUnsafe<T, U>
        where T : unmanaged
        where U : IComparer<T>
    {
        ...
        public SortJobDeferByRefUnsafe(ref UnsafeList<T> data, in U u)
        {
            Data = data;
            Comp = u;
        }
...
#endif
            var deferData = Data.AsDeferredArrayUnsafe();
            
            var segmentSortJob            = new SegmentSortDeferred() { Data = deferData, Comp = Comp, JobNum = threadNum };
            var segementCombine           = segmentSortJob.ScheduleByRef(threadNum, 1, inputDeps);
            var segmentSortMergeJob       = new SegmentSortMergeDeferred() { Data = deferData, Comp = Comp, JobNum = threadNum };
