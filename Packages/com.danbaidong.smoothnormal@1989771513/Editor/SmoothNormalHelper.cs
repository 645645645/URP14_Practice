...
namespace UnityEditor.SmoothNormalTool
{
    public class SmoothNormalHelper
    {
        ...
        public static Vector3[] ComputeSmoothNormal(Mesh mesh, ComputeShader cs, bool toTangentSpace = true, bool useOctahedron = false)
        {
            // Compute
            ...
            cs.SetBool("_SmoothNormalToTangentSpace", toTangentSpace);
            cs.SetBool("_UseOctahedron", useOctahedron);
        ...
        public static void CopyVector3NormalsToNormalXYZ(ref Vector3[] smoothNormals, ref Vector3[] vertexNoramls, int size)
        {
            for (int i = 0; i < size; i++)
            {
                vertexNoramls[i].x = smoothNormals[i].x;
                vertexNoramls[i].y = smoothNormals[i].y;
                vertexNoramls[i].z = smoothNormals[i].z;
            }
        }
        
        public static void CopyVector3NormalsToUV1XYZ(ref Vector3[] smoothNormals, ref List<Vector3> uv1, int size)
        {
            for (int i = 0; i < size; i++)
            {
                uv1.Add(smoothNormals[i]);
            }
        }
        
        public static void CopyVector3NormalsToUVZW(ref Vector3[] smoothNormals, ref List<Vector4> newUv1, ref Vector2[] srcUv0 ,int size)
        {
            for (int i = 0; i < size; i++)
            {
                Vector2 scrUv = srcUv0[i];
                Vector3 octNormal = smoothNormals[i];
                newUv1.Add(new Vector4(scrUv.x, scrUv.y, octNormal.x, octNormal.y));
            }
        }
        
        public static void CopyVector3NormalsToUVXY(ref Vector3[] smoothNormals, ref Vector2[] uv, int size)
        {
            for (int i = 0; i < size; i++)
            {
                Vector3 octNormal = smoothNormals[i];
